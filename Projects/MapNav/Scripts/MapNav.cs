...
public class MapNav : MonoBehaviour 
{
	...
	//AppKey (API key) code obtained from your maps provider (MapQuest, Google, etc.). 
	//Default MapQuest key for demo purposes only (with limitations). Please get your own key before you start yout project.
	//public string key = "Fmjtd%7Cluur29072d%2Cbg%3Do5-908s00";
	public string key = "AIzaSyB_6z0XrSqzoM2npAgUJKijiYuJIf8_SyQ";															
	...
	void Awake(){
		...
		mycam = cam.GetComponent<Camera> ();//Camera.main;
	...
	IEnumerator MapPosition(){
		...
		*/
		...
		multiplier=1;
		url = "http://maps.google.com/maps/api/staticmap?center=" + fixLat + "," + fixLon + "&zoom=" + zoom + "&scale=2&size=640x640&format=jpg&maptype=" + maptype [index] + "&sensor=false&key=AIzaSyB_6z0XrSqzoM2npAgUJKijiYuJIf8_SyQ";//+key;
	...
	IEnumerator Online(){
		if(!mapDisabled){
			...
			while(!www.isDone){
				GameController.Instance.SetLoadingBar(download);
			...
			if(www.error == null){
				...
				GameController.Instance.SetLoadingBar(1f);
				print("Map Ready!");
				//use the status string variable to print messages to your own user interface (GUIText, etc.)
